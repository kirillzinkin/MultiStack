...
{
  ofstream out;
  out.open("MStack.txt", ios::app);
  for (int i = 0; i < n; i++)
  {
    out << endl << "Stack number " << i + 1 << endl;
    stackMas[i]->filens(out);
  }
  cout << endl << endl << "MStack written!!!" << endl;
}

template<class T>
int TMStack<T>::min_kol_elem()
{
  int k = 1;
  for (int i = 1; i < n; i++)
  {
    if((stackMas[i-1]->GetTop()) > (stackMas[i]->GetTop()))
      k = i;
  }
  if((stackMas[n-2]->GetTop()) > (stackMas[n-1]->GetTop()))
    k = n;
  return k;
}
